#
#  Version 2 changes:
#       - Added a No Input intent - due to the fallback intent containing that caused loops sometimes
#       - Google requires that every answer contain a question at the end, or close the connection.
#         I chose to add a question, in case the user wants to hear another rule.
#         Amazon does not have this requirement, only Google
#
      ...
      "messages": [
        {
          "type": 0,
          "lang": "en",
          "speech": "Need info on another rule?"
        }
      ],
   ...
   while line:
       ...
       if (len(str) > 0):
           if (str[0] == '!'):
               ...
               google_says = str.lstrip('=') + '.  Anymore rules?'
               ...
               # No input intent file
               no_inputFile = open(output_path + '/intents/Default NoInput Intent.json', "w")
               no_inputFile.write(no_input_intent)
               no_inputFile.close()
